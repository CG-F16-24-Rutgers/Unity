...
public class Innkeeper : MonoBehaviour
{
    ...
    public Node InnKeep() //root node
    {
        return new DecoratorLoop(new Sequence(this.AssertAndHaveConvo()));
    }

    protected Node AssertAndHaveConvo()
    {
        return new Sequence(
            new DecoratorInvert(new DecoratorLoop(new DecoratorInvert(new Sequence(new LeafAssert(() => egs.in_trigger && egs.col.gameObject.CompareTag("Marker")))))),
    ...
    void Start()
    {
        behaviorAgent = new BehaviorAgent(this.InnKeep());
